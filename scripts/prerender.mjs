...
async function main() {
  ...
  // The Product price must match what the configurator shows: its "from"
  // figure is priceFor("A5", false) — an unmounted A5, natural frame, clear
  // glazing. Computed from the same live pricing tables checkout charges from
  // (server/shop/catalogue.mjs), so a price edit in /admin is reflected on the
  // next deploy rather than drifting against a hardcoded number.
  const livePricing = await fetchPricing(rest);
  // fetchPricing falls back to its hardcoded constants WHOLESALE if the
  // pricing tables can't be read — margin included. The browser doesn't: it
  // reads site_settings.print_margin_percent separately and applies it on top
  // of whatever component costs it managed to load (see fetchPricingSettings
  // in src/lib/supabase.ts). Mirror that, or a margin change would show a
  // different "from $X" in the page than in its own Product JSON-LD.
  const liveMargin = Number(settingValue.get("print_margin_percent"));
  const pricing = Number.isFinite(liveMargin)
    ? { ...livePricing, marginPercent: liveMargin }
    : livePricing;
  const basePrice = (priceCentsFor("A5", false, "natural", "clear", pricing) / 100).toFixed(2);
