...
function GalleriesPage({ onNavigate }: { onNavigate: (route: string) => void }) {
  ...
  // A collection or region scope still needs the query form, since those
  // combinations have no static page of their own. `region` is dropped once a
  // place narrows it further — the place already implies its region.
  useEffect(() => {
  ...
  }, [activeCollection, activeRegion, activeLocation, isLoading, locations]);
